...
func TestSearchField_Kind(t *testing.T) {
	f := hypermedia.SearchField("q", "Search…", "foo")
...
func TestSelectField_KindAndOptions(t *testing.T) {
	opts := []hypermedia.FilterOption{{Value: "a", Label: "A"}}
	f := hypermedia.SelectField("role", "Role", "a", opts)
	if f.Kind != hypermedia.FilterKindSelect {
...
func TestRangeField_MinMaxStep(t *testing.T) {
	f := hypermedia.RangeField("age", "Max age", "30", "18", "100", "1")
	if f.Kind != hypermedia.FilterKindRange {
...
func TestCheckboxField_ValueTrue(t *testing.T) {
	f := hypermedia.CheckboxField("active", "Active", "true")
	if f.Kind != hypermedia.FilterKindCheckbox {
...
func TestCheckboxField_ValueEmpty(t *testing.T) {
	f := hypermedia.CheckboxField("active", "Active", "")
	if f.Value != "" {
...
func TestDateField_Kind(t *testing.T) {
	f := hypermedia.DateField("from", "From", "2024-01-01")
	if f.Kind != hypermedia.FilterKindDate {
...
func TestSelectOptions_CorrectOptionSelected(t *testing.T) {
	opts := hypermedia.SelectOptions("b", "a", "A", "b", "B", "c", "C")
	if len(opts) != 3 {
...
func TestSelectOptions_EmptyCurrentSelectsNothing(t *testing.T) {
	opts := hypermedia.SelectOptions("", "a", "A", "b", "B")
...
func TestSelectOptions_OddPairsHandledSafely(t *testing.T) {
	...
	opts := hypermedia.SelectOptions("", "a", "A", "orphan")
	if len(opts) != 1 {
...
func TestNewFilterBar_DefaultID(t *testing.T) {
	bar := hypermedia.NewFilterBar("/users", "#table-container",
		hypermedia.SearchField("q", "Search", ""),
	)
	if bar.ID != hypermedia.DefaultFilterFormID {
		t.Errorf("expected ID=%q, got %q", hypermedia.DefaultFilterFormID, bar.ID)
	}
	if bar.Action != "/users" {
